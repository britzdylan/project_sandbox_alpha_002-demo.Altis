...
private _buildings = ["minpositions", 2, OSF_MAP_CENTER, 16000] call OSF_fnc_findBuildings;
{
	if (random 1 < _chance) then {
		if (_x getVariable ["OSF_skipDamage", false]) then { continue };
		...
		if (["OSF_debug", false] call OSF_fnc_getMissionVar) then {
			...
			_m setMarkerTypeLocal "loc_Tourism";
		...
		for "_i" from 1 to _numDebris do {
			...
			private _class = if (_dmg > 0.8 && {
				random 1 < 0.05
			}) then {
				selectRandom _wreckClasses
			} else {
				if (random 1 < 0.1) then {
					selectRandom _debrisClasses
				} else {
					""
				}
			};
			...
			if (["OSF_debug", false] call OSF_fnc_getMissionVar) then {
				...
				_m setMarkerTextLocal "Debris";
			};
		};
	};
} forEach _buildings;

["damageBuildings", format ["Damaged %1 of %2 buildings (radius %3m, debris spawned)", _count, count _buildings, 16000]] call OSF_fnc_log;
